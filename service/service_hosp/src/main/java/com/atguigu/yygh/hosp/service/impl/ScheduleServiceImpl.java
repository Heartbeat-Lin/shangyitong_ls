    ...
    public Map<String, Object> getBookingScheduleRule(Integer page, Integer limit, String hoscode, String depcode) {
        ...
        Aggregation agg = Aggregation.newAggregation(
                ...
                Aggregation.group("workDate")//分组字段
                        .first("workDate").as("workDate")
                        .count().as("docCount")
                        .sum("availableNumber").as("availableNumber")   //availableNumber
                        .sum("reservedNumber").as("reservedNumber")
        );
...
//停号时间
    ...
    public ScheduleOrderVo getScheduleOrderVo(String scheduleId) {
        ...
        //获取预约规则信息
        Hospital hospital = hospitalService.getHosByHoscode(schedule.getHoscode());
        if(null == hospital) {
            throw new YyghException(ResultCodeEnum.DATA_ERROR);
        }
        BookingRule bookingRule = hospital.getBookingRule();
    ...
    private IPage<Date> getListDate(Integer page, Integer limit, BookingRule bookingRule) {
        ...
        for (int i = 0; i < cycle; i++) {
            ...
            String dateString = curDateTime.toString();
